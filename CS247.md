...
int old[5] = { 21, 31, 49, 16, 43 };
int new[5];

transform(old.begin(), old.end(), new.begin(), std::bind1st(add, 10));
// new = { 31, 41, 59, 26, 53 };
```

#Lambda Functions

**Lambda functions** are small, in-line functions that can be used instead of a function or a functor. This allows disposable functions to be defined, which is useful for algorithms.  Lambda functions have the following form:
